...
import { useGetResultsQuery, useCreateResultMutation, useUpdateResultMutation, useDeleteResultMutation } from "@/state/api";
...
const Results = () => {
  ...
  const handleUpdateResult = async (id: number, resultData: any) => {
    try {
      await updateResult({ id, data: resultData }).unwrap();
      setIsModalOpen(false);
      setEditingResult(null);
    } catch (error) {
              ...
              results.map((result: any) => (
                <TableRow key={result.id} className="hover:bg-gray-50 dark:hover:bg-gray-900">
                  <TableCell className="whitespace-nowrap font-medium">
                    {result.student ? `${result.student.name} ${result.student.surname}` : "N/A"}
                  </TableCell>
                  ...
                  <TableCell className="whitespace-nowrap">
                    {result.exam ? "Exam" : result.assignment ? "Assignment" : "N/A"}
                  </TableCell>
                  <TableCell className="whitespace-nowrap">
